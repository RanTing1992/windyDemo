...
CanvasWindy.prototype = {
  ...
  _init: function () {
    ...
    console.log(this.extent, "新的风场范围---", this.extent.length);
    if (this.extent.length != 0) {
      ...
      this.generateParticleExtent.push(
        ((this.extent[0] + 180 - west) / (east - west)) * (nx - 2)
      ); //左
      this.generateParticleExtent.push(
        ((this.extent[1] + 180 - west) / (east - west)) * (nx - 2)
      ); //右
      this.generateParticleExtent.push(
        ((north - this.extent[2]) / (north - south)) * (ny - 2)
      ); //下
      this.generateParticleExtent.push(
        ((north - this.extent[3]) / (north - south)) * (ny - 2)
      ); //上
    }
    // 创建风场粒子
  ...
  //根据现有参数重新生成风场
  redraw: function (extentArr) {
    window.cancelAnimationFrame(this.animateFrame);
    this.particles = [];
    this.extent = extentArr;
    ...
    do {
      x =
        this.generateParticleExtent.length == 0
          ...
          : this.fRandomByfloat(
              this.generateParticleExtent[0],
              this.generateParticleExtent[1]
            );
      y =
        this.generateParticleExtent.length == 0
          ? this.fRandomByfloat(0, this.windField.rows - 2)
          : this.fRandomByfloat(
              this.generateParticleExtent[3],
              this.generateParticleExtent[2]
            );
